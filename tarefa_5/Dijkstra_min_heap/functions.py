# projeto_dijkstra_min_heap/functions.py
...
# Não é necessária uma heurística específica para Dijkstra padrão
...
# Usa Dijkstra otimizado do NetworkX (com min-heap)
def calcular_distancia_rota(G, no_origem, no_destino):
    """Calcula a distância da rota mais curta usando Dijkstra (otimizado com min-heap) e retorna a distância."""
    try:
        return nx.shortest_path_length(G, no_origem, no_destino, weight='length')
...
def plotar_mapa_com_rotas(graph_osmnx, destinos, czoonoses, rotas_completas, labels_clusters):
    ...
    for i, route_data in rotas_completas.items(): 
        ...
        # CORREÇÃO: ox.routing.route_to_gdf
