...
from functions import (
    ...
    heuristic # Importa a heurística para usar no nx.astar_path
)
...
if __name__ == "__main__":
    ...
    tracker = EmissionsTracker(
        project_name='Rotas_Astar', # Nome do seu projeto A*
        output_dir='./', # Onde o arquivo de saída será salvo
        output_file='emissions_astar.csv', # Nome do arquivo de saída
    ...
    try: # NOVO: Envolva seu código principal em um try-finally para garantir que o tracker pare
        ...
        for cluster_id, cluster_pontos in pontos_por_cluster.items():
            ...
            if len(tour_otimizado) > 1:
                for i in range(len(tour_otimizado) - 1):
                    ...
                    if distancia_segmento != float('inf'):
                        ...
                        # USANDO NX.ASTAR_PATH COM A LAMBDA
                        path_segmento = nx.astar_path(G, origem, destino, heuristic=lambda u, v: heuristic(G, u, v), weight='length')
            ...
            # USANDO NX.ASTAR_PATH COM A LAMBDA
            rota_ida_nos = nx.astar_path(G, no_czo, ponto_distante[1], heuristic=lambda u, v: heuristic(G, u, v), weight='length')
            # USANDO NX.ASTAR_PATH COM A LAMBDA
            rota_volta_nos = nx.astar_path(G, ultimo_ponto[1], no_czo, heuristic=lambda u, v: heuristic(G, u, v), weight='length')
